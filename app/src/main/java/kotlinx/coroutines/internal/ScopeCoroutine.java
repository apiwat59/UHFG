...
public class ScopeCoroutine<T> extends AbstractCoroutine<T> implements CoroutineStackFrame {
    ...
    protected void afterCompletionInternal(Object state, int mode) {
        if (state instanceof CompletedExceptionally) {
            Throwable exception = mode == 4 ? ((CompletedExceptionally) state).cause : StackTraceRecoveryKt.recoverStackTrace(((CompletedExceptionally) state).cause, this.uCont);
            ResumeModeKt.resumeUninterceptedWithExceptionMode(this.uCont, exception, mode);
        } else {
