...
final /* synthetic */ class FlowKt__ContextKt {
    ...
    public static final <T> Flow<T> buffer(Flow<? extends T> buffer, int capacity) {
        ...
        if (buffer instanceof ChannelFlow) {
            return ChannelFlow.update$default((ChannelFlow) buffer, null, capacity, 1, null);
        }
        return new ChannelFlowOperatorImpl(buffer, null, capacity, 2, null);
    }
    ...
    public static final <T, R> Flow<R> flowWith(final Flow<? extends T> flowWith, final CoroutineContext flowContext, final int bufferSize, final Function1<? super Flow<? extends T>, ? extends Flow<? extends R>> builder) {
        ...
        return new Flow<R>() { // from class: kotlinx.coroutines.flow.FlowKt__ContextKt$flowWith$$inlined$unsafeFlow$1
            ...
            public Object collect(final FlowCollector collector, Continuation $completion) {
                CoroutineContext originalContext = $completion.get$context().minusKey(Job.INSTANCE);
                Flow prepared = FlowKt.buffer(FlowKt.flowOn(Flow.this, originalContext), bufferSize);
                Flow $this$collect$iv = FlowKt.buffer(FlowKt.flowOn((Flow) builder.invoke(prepared), flowContext), bufferSize);
                return $this$collect$iv.collect(new FlowCollector<R>() { // from class: kotlinx.coroutines.flow.FlowKt__ContextKt$flowWith$$inlined$unsafeFlow$1$lambda$1
                    @Override // kotlinx.coroutines.flow.FlowCollector
                    public Object emit(Object value, Continuation $completion2) {
                        return FlowCollector.this.emit(value, $completion2);
                    }
                }, $completion);
            }
        };
