...
final /* synthetic */ class FlowKt__LimitKt {
    public static final <T> Flow<T> drop(Flow<? extends T> drop, int count) {
        Intrinsics.checkParameterIsNotNull(drop, "$this$drop");
        if (count >= 0) {
            return new FlowKt__LimitKt$drop$$inlined$unsafeFlow$1(drop, count);
        }
    ...
    public static final <T> Flow<T> take(Flow<? extends T> take, int count) {
        Intrinsics.checkParameterIsNotNull(take, "$this$take");
        if (count > 0) {
            return new FlowKt__LimitKt$take$$inlined$unsafeFlow$1(take, count);
        }
