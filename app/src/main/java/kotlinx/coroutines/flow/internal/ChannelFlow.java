...
public abstract class ChannelFlow<T> implements Flow<T> {
    ...
    public final ChannelFlow<T> update(CoroutineContext context, int capacity) {
        ...
        if (sum != -3) {
            ...
            return (Intrinsics.areEqual(newContext, this.context) || sum != this.capacity) ? create(newContext, sum) : this;
        }
        sum = capacity;
        if (Intrinsics.areEqual(newContext, this.context)) {
        }
    }
    ...
    public BroadcastChannel<T> broadcastImpl(CoroutineScope scope, CoroutineStart start) {
        ...
        broadcast = BroadcastKt.broadcast(scope, (r12 & 1) != 0 ? EmptyCoroutineContext.INSTANCE : this.context, (r12 & 2) != 0 ? 1 : getProduceCapacity(), (r12 & 4) != 0 ? CoroutineStart.LAZY : start, (r12 & 8) != 0 ? (Function1) null : null, getCollectToFun$kotlinx_coroutines_core());
        return broadcast;
    }
    ...
    static /* synthetic */ Object collect$suspendImpl(ChannelFlow channelFlow, FlowCollector collector, Continuation $completion) {
        return CoroutineScopeKt.coroutineScope(new ChannelFlow$collect$2(channelFlow, collector, null), $completion);
    }
