...
final class AwaitAll<T> {
    ...
    public final Object await(Continuation<? super List<? extends T>> continuation) {
        ...
        for (int i = 0; i < length; i++) {
            int i2 = Boxing.boxInt(i).intValue();
            Deferred deferred = this.deferreds[i2];
            deferred.start();
            AwaitAllNode $this$apply = new AwaitAllNode(this, cont, deferred);
            AwaitAllNode $this$asHandler$iv = $this$apply;
            $this$apply.setHandle(deferred.invokeOnCompletion($this$asHandler$iv));
            nodes[i] = $this$apply;
        }
    ...
    private final class DisposeHandlersOnCancel extends CancelHandler {
        ...
        public DisposeHandlersOnCancel(AwaitAll $outer, AwaitAll<T>.AwaitAllNode[] nodes) {
    ...
    private final class AwaitAllNode extends JobNode<Job> {
        ...
        /* JADX WARN: Multi-variable type inference failed */
        public AwaitAllNode(AwaitAll $outer, CancellableContinuation<? super List<? extends T>> continuation, Job job) {
            super(job);
        ...
        public void invoke2(Throwable cause) {
            if (cause != null) {
                ...
                if (token != null) {
                    ...
                    DisposeHandlersOnCancel disposer = this.disposer;
                    if (disposer != null) {
                        disposer.disposeAll();
                        return;
                    }
            ...
            if (AwaitAll.notCompletedCount$FU.decrementAndGet(this.this$0) == 0) {
                ...
                cancellableContinuation.resumeWith(Result.m16constructorimpl((List) destination$iv$iv));
            }
        }
    }
}
