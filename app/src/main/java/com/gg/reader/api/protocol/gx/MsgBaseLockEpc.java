...
public class MsgBaseLockEpc extends Message {
    ...
    public MsgBaseLockEpc(byte[] data) {
        this();
        if (data == null) {
            return;
        }
    ...
    public void pack() {
        ...
        if (!StringUtils.isNullOfEmpty(this.hexPassword)) {
            buffer.putInt(2, 8);
            byte[] tmpByte2 = HexUtils.hexString2Bytes(this.hexPassword);
            buffer.put(tmpByte2);
        }
        byte[] tmpByte3 = buffer.asByteArray();
        this.cData = tmpByte3;
    ...
    public void ackUnpack() {
        Hashtable<Byte, String> dicErrorMsg = new Hashtable<Byte, String>() { // from class: com.gg.reader.api.protocol.gx.MsgBaseLockEpc.1
            {
                put((byte) 0, "Success.");
