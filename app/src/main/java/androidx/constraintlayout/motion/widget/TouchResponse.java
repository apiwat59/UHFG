...
class TouchResponse {
    ...
    void processTouchRotateEvent(MotionEvent event, MotionLayout.MotionTracker velocityTracker, int currentState, MotionScene motionScene) {
        ...
        if (action == 1) {
            ...
            float relativePosY2 = event.getRawY() - rcy;
            double angle1 = Math.toDegrees(Math.atan2(relativePosY2, relativePosX));
            int i4 = this.mTouchAnchorId;
            if (i4 != -1) {
                relativePosY = relativePosY2;
            ...
            this.mMotionLayout.touchAnimateTo(this.mOnTouchUp, target, 3.0f * angularVelocity2);
        ...
        if (action == 2) {
            ...
            float relativePosY3 = event.getRawY() - rcy2;
            ...
            if (Math.abs(drag) > 0.01d || this.mDragStarted) {
                ...
                if (pos3 == current) {
                    ...
                    double angle23 = tvx2;
                    this.mMotionLayout.mLastVelocity = (float) Math.toDegrees((float) ((Math.hypot(tvy2, angle23) * Math.sin(Math.atan2(tvy2, tvx2) - angle12)) / Math.hypot(relativePosX2, relativePosY3)));
                }
                this.mLastTouchX = event.getRawX();
                this.mLastTouchY = event.getRawY();
            }
