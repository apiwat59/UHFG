...
public abstract class WidgetRun implements Dependency {
    ...
    protected final DependencyNode getTarget(ConstraintAnchor anchor) {
        ...
        int i = AnonymousClass1.$SwitchMap$androidx$constraintlayout$core$widgets$ConstraintAnchor$Type[targetType.ordinal()];
        if (i == 1) {
    ...
    protected final DependencyNode getTarget(ConstraintAnchor anchor, int orientation) {
        ...
        if (i != 1) {
            ...
            DependencyNode target = run.end;
            return target;
        }
        DependencyNode target2 = run.start;
        return target2;
    }
