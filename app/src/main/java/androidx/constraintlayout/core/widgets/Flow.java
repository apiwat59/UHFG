    ...
    */
    ...
    private class WidgetsList {
        ...
        public void createConstraints(boolean isInRtl, int chainIndex, boolean isLastChain) {
            ...
            for (int i6 = 0; i6 < count && this.mStartIndex + i6 < Flow.this.mDisplayedWidgetsCount; i6++) {
                ConstraintWidget widget5 = Flow.this.mDisplayedWidgets[this.mStartIndex + i6];
                if (widget5 != null) {
                    if (i6 == 0) {
    ...
    public void addToSolver(LinearSystem system, boolean optimize) {
        ...
        if (i != 0) {
            if (i == 1) {
                ...
                int i2 = 0;
                while (i2 < count) {
                    WidgetsList list = this.mChainList.get(i2);
                    list.createConstraints(isInRtl, i2, i2 == count + (-1));
                    i2++;
                }
            } else if (i == 2) {
                ...
                int i3 = 0;
                while (i3 < count2) {
                    WidgetsList list2 = this.mChainList.get(i3);
                    list2.createConstraints(isInRtl, i3, i3 == count2 + (-1));
                    i3++;
                }
            }
        } else if (this.mChainList.size() > 0) {
            WidgetsList list3 = this.mChainList.get(0);
            list3.createConstraints(isInRtl, 0, true);
        }
