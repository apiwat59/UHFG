...
public final class SpringForce implements Force {
    ...
    DynamicAnimation.MassState updateValues(double lastDisplacement, double lastVelocity, long timeElapsed) {
        ...
        if (displacement2 > 1.0d) {
            ...
            double displacement3 = Math.pow(2.718281828459045d, (-d6) * deltaT) * ((coeffB2 * deltaT) + lastDisplacement2);
            ...
            double displacement4 = Math.pow(2.718281828459045d, (-displacement2) * d9 * deltaT) * ((Math.cos(this.mDampedFreq * deltaT) * lastDisplacement2) + (Math.sin(this.mDampedFreq * deltaT) * sinCoeff));
            double d10 = this.mNaturalFreq;
            double lastDisplacement3 = this.mDampingRatio;
            double d11 = (-d10) * displacement4 * lastDisplacement3;
            ...
            double cos = d11 + (pow3 * (sin + (d13 * sinCoeff * Math.cos(d13 * deltaT))));
