...
class FragmentTransition {
    ...
    static void startTransitions(FragmentManager fragmentManager, ArrayList<BackStackRecord> records, ArrayList<Boolean> isRecordPop, int startIndex, int endIndex, boolean isReordered, Callback callback) {
        ...
        int i3 = transitioningFragments.size();
        if (i3 != 0) {
            View nonExistentView = new View(fragmentManager.mHost.getContext());
            int numContainers2 = transitioningFragments.size();
            int i4 = 0;
            while (i4 < numContainers2) {
                int containerId = transitioningFragments.keyAt(i4);
                ArrayMap<String, String> nameOverrides = calculateNameOverrides(containerId, records, isRecordPop, startIndex, endIndex);
                FragmentContainerTransition containerTransition = transitioningFragments.valueAt(i4);
                if (isReordered) {
    ...
    private static void configureTransitionsReordered(FragmentManager fragmentManager, int containerId, FragmentContainerTransition fragments, View nonExistentView, ArrayMap<String, String> nameOverrides, final Callback callback) {
        ViewGroup sceneRoot;
        Fragment inFragment;
    ...
    private static void configureTransitionsOrdered(FragmentManager fragmentManager, int containerId, FragmentContainerTransition fragments, View nonExistentView, ArrayMap<String, String> nameOverrides, final Callback callback) {
        ViewGroup sceneRoot;
        Fragment inFragment;
        final Fragment outFragment;
        FragmentTransitionImpl impl;
