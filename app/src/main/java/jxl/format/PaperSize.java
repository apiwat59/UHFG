...
public final class PaperSize {
    ...
    private PaperSize(int v, boolean growArray) {
        ...
        if (v >= paperSizeArr.length && growArray) {
            PaperSize[] newarray = new PaperSize[v + 1];
            System.arraycopy(paperSizeArr, 0, newarray, 0, paperSizeArr.length);
            paperSizes = newarray;
        }
        PaperSize[] paperSizeArr2 = paperSizes;
        if (v < paperSizeArr2.length) {
            paperSizeArr2[v] = this;
        }
    }
    ...
    public static PaperSize getPaperSize(int val) {
        PaperSize[] paperSizeArr = paperSizes;
        PaperSize p = val > paperSizeArr.length + (-1) ? null : paperSizeArr[val];
